...
- 「添加原文链接」默认勾选（旧配置同样默认开启），保存剪贴设置后，快捷键及手动新剪贴均在正文顶部添加「原文地址」和可点击的文章标题。取消勾选后不添加该段。已保存的文档不自动覆盖；待处理任务重试沿用创建时的选项。
...
- 对照油猴脚本 `src/clipper/post-processor.ts` 的 `sourceLinker`，将原文地址放到正文顶部，以文章标题作为可点击链接。
- 新配置及旧配置缺省均默认开启；关闭后，正文不添加来源段。每个任务保存创建时的选项，重试不因后续设置变化而改变。
- 独立思源真实创建开启／关闭两篇文档并导出检查，分别确认来源段位于正文前、正文没有附加来源段。
- 本机界面默认勾选；实际取消勾选并保存，配置回读为 `false`；重新勾选并保存，配置回读为 `true`，最终保留开启状态。
- 更新后类型检查、格式及 lint 通过；全仓 566 个测试通过、2 个跳过。已更新本机 FoLocal。
