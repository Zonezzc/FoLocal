...
- 1.13.3 新增源网页剪贴到思源：支持静态及动态正文、预览、图片本地化、保存回读、去重和失败重试；桌面阅读全文改为直接抓取源网页，不使用 RSS 摘要代替正文。
- 1.13.4 修复思源剪贴图标缺失，新增可自定义的 `Ctrl⇧C` 快捷键（Mac 使用 Control 键）；Windows/Linux 默认 `Ctrl⇧S`，避免与原有复制链接快捷键冲突。文章菜单同时显示按键提示。
